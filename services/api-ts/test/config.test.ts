...
test("valid values are read, in the formats api-go accepts", () => {
  ...
  const timeouts: [string, number][] = [["500ms", 500], ["1m30s", 90_000], ["1.5s", 1500], ["2h", 7_200_000]];
...
test("values that cannot be used are refused", () => {
  const bad = [
    ...
    { SHUTDOWN_TIMEOUT: "10" }, { SHUTDOWN_TIMEOUT: "0s" }, { SHUTDOWN_TIMEOUT: "1d" }, { SHUTDOWN_TIMEOUT: "1.s" }, { SHUTDOWN_TIMEOUT: "-1s" },
  ];
