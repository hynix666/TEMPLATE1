...
6. **Keep the contract in step** — the API table lives in `services/api-py/README.md`, and every service present must answer each route with the same status codes; `services/api-go/README.md` is the reference table. If the endpoint changes who depends on whom, update `architecture/model/system.c4` when it exists.
