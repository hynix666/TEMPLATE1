...
6. **Keep the contract in step** — the API table lives in `services/api-go/README.md` when that service exists; both services must answer every route with the same status codes. If the endpoint changes who depends on whom, update `architecture/model/system.c4` when it exists.
