...
6. **Keep the contract in step** — update the API table in `services/api-go/README.md`. If `services/api-ts` exists, make the same change there: both services must answer every route with the same status codes. If the endpoint changes who depends on whom, update `architecture/model/system.c4` when it exists.
