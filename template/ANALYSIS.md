...
**What building it found.** Five defects, each caught by a check before it could ship: the Python boundary checker did not resolve relative imports (caught by its must-fire test); `.gitignore` had no Python rules, so a first run committed 17 bytecode files (now a hygiene rule); the preset job installed no uv (caught by generating every preset in CI); the ADR template linked a placeholder path (caught by the new link rule on its first run); and `api-ts`'s README linked into `api-go`'s, which a project without the Go service does not have (caught by the same rule, in the `fullstack-ts` preset — each module now carries its own API table).

## Fifth round: Coolify (template v1.5.0)

*Added 18 September 2026.* Coolify is a self-hosted deployment platform: a Laravel application of about 4,100 files, not a template. It was read for what a large, active open-source project does about contributions, releases, images and agents.

| Area | What Coolify does | Taken into TEMPLATE1 | Left out, and why |
|---|---|---|---|
| Container images | Builds each image on native `amd64` and `arm64` runners and joins them with `docker buildx imagetools create` | `mcp-publish.yml` does the same for the MCP server image, the one image the template publishes, with no new third-party action | Its per-image Discord notifications and untagged-image cleanup job |
| Agent lessons | `.ai/lessons.md`, a short list of mistakes agents made there and the rule that prevents each | The most general one, reproduce a bug with a failing test before fixing it, as a line in `AGENTS.md` — not a second instructions file | The rest are about its own UI framework |
| Pull requests | A template with an "AI assistance" disclosure; an automated gate (`anti-slop`) on PR descriptions, account age and blocked phrases | The disclosure section, as a request rather than a rule | The gate: an unpinned third-party action on `pull_request_target` with write access, rejecting PRs whose text names an AI tool |
| Contributing | "Discussion is required for larger changes" | The same, as the first step of the loop in `CONTRIBUTING.md` | "What this project is not": only a real project can write it |
| Releases | `main`/`next`/`hotfix` branches, release candidates, a `versions.json` the running product reads | Nothing | Product release engineering; release-please on trunk covers what a new project needs |
| Changelog | git-cliff, committed by a bot pushing straight to `main` | Nothing | It needs a push that bypasses the ruleset every project here has |
| Agent tooling | Laravel Boost writes guidelines into `AGENTS.md` and copies twelve skills into `.agents/`, `.claude/` and `.cursor/`; `skills-lock.json` pins an external skill by hash | Nothing | The copies have already drifted (one folder lacks a skill the others have), which is what [ADR-0009](../docs/adr/0009-where-agent-adapters-and-skills-live.md) prevents. The lock file is a good idea for a project that imports skills; this template has none to pin |
...
## Seventh round: ECC and ponytail (template v1.7.0)

*Added 19 September 2026.* Both are agent toolkits distributed to many coding assistants, not application templates. They were read for what they do about supply chain and agent safety.

| Source | Strength | Taken into TEMPLATE1 | Left out, and why |
|---|---|---|---|
| ECC (Everything Claude Code) | Every action pinned; `npm ci --ignore-scripts` and `npm audit signatures` in CI; a check that fails on invisible Unicode, including the tag block used to hide instructions from human reviewers; a check against personal absolute paths; SLSA provenance for its releases | `--ignore-scripts` on every npm install (in `setup.mjs`, the workflows and the Dockerfiles, enforced by `check-hygiene` rule 8); `npm audit signatures` after each install in `verify`; `check-hygiene` rules 11 (invisible and text-reordering characters) and 12 (paths into a home directory) | Its IOC scanner for named malware, a list that goes stale; install manifests with modules, components and profiles, where `features.json` already selects features; SLSA provenance, since npm publishing already carries provenance; the configuration for about fifteen assistants |
| ponytail | A persona skill shipped to about ten assistants, with a script keeping the copies identical | Nothing | Unpinned workflows; paid benchmark runs; per-assistant copies, the arrangement [ADR-0009](../docs/adr/0009-where-agent-adapters-and-skills-live.md) avoids |

**What building it found.** Rule 11's first run flagged the file that defines it: the edit that added the character class had written the characters themselves rather than their escapes, so the rule's own source hid invisible characters. It is written from escapes now. Rule 12 likewise flagged its own test, whose fixtures spelt out home directories; they are assembled at run time. `--ignore-scripts` changed nothing that works: only `fsevents` (macOS, optional) and `esbuild`'s binary check have install scripts in any lockfile, and every Node module passed its own `verify` with them skipped.

## Eighth round: dspy and plane (template v1.8.0)

*Added 19 September 2026.* plane was the same snapshot round four read, so only what that round passed over was considered. dspy is new: a mature Python library, read for its release and test workflows.

| Source | Strength | Taken into TEMPLATE1 | Left out, and why |
|---|---|---|---|
| dspy | Every action pinned; zizmor and actionlint as the first CI job, found independently of round six; publishing through GitHub deployment environments (`pypi`, `testpypi`) with trusted publishing; the built package installed into a fresh environment and exercised before it is published; tests across Python 3.10 to 3.14 and at the lowest dependency versions it declares | Deployment environments for both publishing jobs (`npm` in `release.yml`, `mcp-registry` in `mcp-publish.yml`); an install check in `ts-library`'s `verify` that packs the library, installs the tarball into an empty project and imports it by name | The Python version matrix and the lowest-version resolution, which protect a library's declared range; `py-service` is a service on one Python version with no runtime dependencies. A TestPyPI rehearsal, which the install check replaces without a second registry |
| plane | Trivy scanning, with a `.trivyignore` that states the reason for each suppression | A report-only `image-scan` job in `security.yml`: every image the repository's Dockerfiles build is scanned for fixable high and critical vulnerabilities, the operating system's packages included, by a checksum-pinned Trivy CLI rather than its GitHub Action; `.trivyignore` with the same rule | Its copyright-header check, migration and i18n checks: product policy |

**What the round confirmed.** Two unrelated, well-run repositories — dspy and Linear's SDK — gate on zizmor and actionlint, as this template has since v1.6.0. The gap this round closed was one the other audits could not see: they read lockfiles, and an image also carries the packages of the operating system it is built on.
